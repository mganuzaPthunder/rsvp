...
The glitter rain is decorative and hidden entirely
...
scatter rather than a blizzard; pass `count` to `<GlitterRain />` to override.
...
RSVP turns over on its own 5s loop (`flipS`). Both live on separate elements
